...
protected:
    string calc(string input) {
        ...
        blake(input_binary, input.size()/2, digest_binary); ///, digest_size);
        delete[] input_binary;
...
TEST_P(BlakeTest, defaultFixturesTest) {
...
auto blakeFixtures = FixturesCSVLoader::load("data/blake.csv");
INSTANTIATE_TEST_CASE_P(defaultFixturesTest, BlakeTest, ::testing::ValuesIn(blakeFixtures));
